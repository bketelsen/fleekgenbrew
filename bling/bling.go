...
func (b *Bling) ConfigFiles() map[string]string {
	configfiles := make(map[string]string)
	for _, prog := range b.ProgramMap {
		if len(prog.ConfigFiles) > 0 {
			for _, file := range prog.ConfigFiles {
				configfiles[file.FileName] = file.Value
			}
		}
	}
	return configfiles
